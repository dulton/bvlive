...
typedef struct AVInputFormat {
    ...
    /**
     * Read one frame and put it in 'frm'. pts and flags are also
     * set. 'avformat_new_stream' can be called only if the flag
     * AVFMTCTX_NOHEADER is used and only in the calling thread (not in a
     * background thread).
     * @return 0 on success, < 0 on error.
     ...
     *         or must be freed before returning
     */
    int (*read_frame)(struct AVFormatContext *, AVFrame *frame);
...
/**
 * Return the next uncoded frame from hard encoder
 * s->iformat should support read_frame function otherwise return < 0
 *
 * @return 0 if OK, < 0 on error
 */
int av_read_uncoded_frame(AVFormatContext *s, AVFrame *frame);
